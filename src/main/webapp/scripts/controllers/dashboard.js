...
angular.module('yapp')
  .controller('DashboardCtrl', function($scope, $state, $rootScope, APIService) {
    ...
    $scope.launchInstance = function (instance) {
        ...
        APIService.launchInstance(instance.name, instance.flavor, instance.image, instance.network, function(response) {
            if(response.data) {
	...
	APIService.getNetworkList(function(response) {
        if(response.data) {
...
angular.module('yapp').controller('AlertCtrl', function($scope, $location) {
    $scope.submit = function(event){
    //	alert(event.target);
    //	console.log(event.target);
    //	alert(event.target.elements["email"].value);
    	emailjs.sendForm('gmail', 'vstack', event.target);
    	alert("Alert sent");
      	return false;
